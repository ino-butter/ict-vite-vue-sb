<template>
	<div>
		<div class="max-w-6xl mx-auto my-5 bg-white rounded-lg shadow-md overflow-hidden">
			...
			<div v-if="onSeatArea" class="max-w-6xl mx-auto my-5 bg-[#fdfbf5] rounded-lg shadow-md border border-gray-300 p-4">
    
    <!-- 스크린 표시 -->
    <div class="flex justify-center mb-4">
      <div class="w-full h-6 bg-gray-700 rounded text-white flex items-center justify-center font-bold">
        SCREEN
      </div>
    </div>
      ...
      <div v-for="(row, rowIndex) in seats" :key="rowIndex" class="flex gap-2 justify-center">
        <div
          ...
          :class="[
            ...
            seat.SEAT_TYPE === 'empty' ? 'bg-gray-100 border-gray-300' :
            seat.SEAT_TYPE === 'screen' ? 'bg-black text-white border-black' :
            seat.SEAT_TYPE === 'vip' ? 'bg-yellow-400 border-yellow-500' :
            seat.SEAT_TYPE === 'couple' ? 'bg-pink-400 border-pink-500' :
			seat.SEAT_TYPE === 'entrance' ? 'bg-green-400 border-green-500' :
			seat.SEAT_TYPE === 'exit' ? 'bg-blue-400 border-blue-500' :
    ...
    <div class="flex justify-center gap-4 mt-4 text-xs">
      <div class="flex items-center gap-1"><div class="w-4 h-4 bg-green-300 border-green-400 border"></div> 일반석</div>
      <div class="flex items-center gap-1"><div class="w-4 h-4 bg-yellow-400 border-yellow-500 border"></div> VIP석</div>
      <div class="flex items-center gap-1"><div class="w-4 h-4 bg-pink-400 border-pink-500 border"></div> 커플석</div>
      <div class="flex items-center gap-1"><div class="w-4 h-4 bg-gray-100 border-gray-300 border"></div> 빈자리</div>
    </div>
